...
export class DashboardComponent implements OnInit {
  ...
  ngOnInit() {
    const data= localStorage.getItem('jsonloaded');
    console.log(data);
    ...
    if (data!='true') {
      this.dataService.getJsonData().subscribe(
        (data) => {
          ...
          /* this.dataService.postData(this.jsonData).subscribe(
            (response) => {
          ...
          chunks.forEach((chunk, index) => {
            this.dataService.postData(chunk).subscribe(
              (response) => {
                console.log(`Post successful for chunk ${index + 1}:`, response);
                this.getAllData();
                // Handle successful post response
              },
              (error) => {
                console.error(
                  `Error posting data for chunk ${index + 1}:`,
                  error
                );
                // Handle error response
              }
